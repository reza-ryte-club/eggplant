 ...
 */
  ...
  strcpy(Fname1,"Test.seq");
  strcpy(Fname2,"New_KS.267.seq");
  strcpy(Fname3,"New_KS.267.obs");
...
{
  int ires;
  int nlines, nl,helix_counter=0,sheet_counter=0,coil_counter=0, total_counter=0;
/*
 ...
 */
  ...
  printf("    %s\n","Predicted Sec. Struct.");
 printf("\n\nSequence length %d",nres);
  printf("\n\n");

  if(fp2 != NULL) {
    //fprintf(fp2,"\n\n%s\n%d\n",title+1,nres);
    printf("SEQ\tPRD\tH\tE\tC\n");
    for(ires = 1; ires <= nres; ires++){
    printf(" %c\t%c\t%.0f\t%.0f\t%.0f\n",seq[ires],predi[ires],roundf(proba[ires][1]*1000),roundf(proba[ires][2]*1000),roundf(proba[ires][3]*1000));

    if(predi[ires]=='H'){helix_counter++;total_counter++;}
    if(predi[ires]=='E'){sheet_counter++;total_counter++;}
    if(predi[ires]=='C'){coil_counter++;total_counter++;}
    }
    printf("\n\nGORIV\n");
    printf("Alpha helix     (H) :     %d is   %.2f%%\n",helix_counter,(float)(helix_counter*100)/total_counter);
    printf("Extended Strand (E) :     %d is   %.2f%%\n",sheet_counter,(float)(sheet_counter*100)/total_counter);
    printf("Random Coil     (C) :     %d is   %.2f%%\n",coil_counter,(float)(coil_counter*100)/total_counter);
   //310  helix       (Gg) :     0 is   0.00%
   //Pi helix        (Ii) :     0 is   0.00%
   //Beta bridge     (Bb) :     0 is   0.00%
   //Extended strand (Ee) :     9 is  69.23%
   //Beta turn       (Tt) :     0 is   0.00%
   //Bend region     (Ss) :     0 is   0.00%
   //Random coil     (Cc) :     4 is  30.77%
